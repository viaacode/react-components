...
describe('<ControlBar />', () => {
	...
	it('gives the volume dropdown a unique id per instance, instead of a shared hardcoded one', () => {
		...
		const { container } = render(
			<>
				<ControlBar {...buildProps()} />
				<ControlBar {...buildProps()} />
			</>
		);

		const flyouts = container.querySelectorAll('.c-flowplayer-volume-flyout');
		...
		expect(ids[0]).not.toEqual('flowplayer-controls__volume');
	});
});
